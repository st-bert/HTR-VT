...
class SimpleAttentionDecoder(nn.Module):
    def __init__(self, embed_dim, max_seq_length, total_classes, num_heads=8):
        super().__init__()
        self.max_seq_length = max_seq_length
        self.embed_dim = embed_dim
        
        # Learnable query tokens for sequence positions
        self.query_tokens = nn.Parameter(torch.zeros(1, max_seq_length, embed_dim))
        
        # Positional encoding for query tokens (helps with position awareness)
        ...
        # Feed-forward layer for additional expressivity (optional)
    ...
    def forward(self, x):
        ...
        # Repeat query tokens for each item in the batch and add positional encoding
        query = self.query_tokens.expand(batch_size, -1, -1)
        query = query + self.pos_embed
        
        # Cross-attention between query tokens and encoder features
        attn_output, _ = self.cross_attention(query, x, x)
        out = query + attn_output  # Residual connection
        out = self.norm1(out)
        
        # Optional feed-forward network with residual connection
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self,
        ...
        # Simple attention decoder for sequence generation
        self.sequence_decoder = SimpleAttentionDecoder(
            embed_dim=embed_dim,
