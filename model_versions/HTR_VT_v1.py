...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self,
        ...
        self.seq_decoder = nn.ModuleList([
            Block(embed_dim, num_heads, self.num_patches,
                  mlp_ratio, qkv_bias=True, norm_layer=norm_layer)
            for i in range(2)])  # Using 2 transformer blocks for sequence decoding
            
        # Output projection for each position in the sequence
        self.seq_head = nn.Linear(embed_dim, self.total_classes)
        
        # Position-wise projection
        self.pos_projections = nn.ModuleList([
            nn.Linear(embed_dim, embed_dim) for _ in range(max_seq_length)
        ])
    ...
    def forward(self, x, mask_ratio=0.0, max_span_length=1, use_masking=False):
        ...
        # Instead of global average pooling, use a sequence-aware approach
        # Collapse the feature map horizontally (assuming text runs horizontally)
        # This preserves the sequential nature of the features
        
        # Create a learnable query embedding for each sequence position
        seq_queries = torch.stack([
            self.pos_projections[i](torch.ones(b, 1, self.embed_dim, device=x.device))
            for i in range(self.max_seq_length)
        ], dim=1)  # [batch_size, max_seq_length, 1, embed_dim]
        
        seq_queries = seq_queries.squeeze(2)  # [batch_size, max_seq_length, embed_dim]
        
        # Generate sequence predictions using cross-attention
        # Each query attends to the entire encoded image
        seq_outputs = []
        
        for i in range(self.max_seq_length):
            # Extract query for this position
            pos_query = seq_queries[:, i:i+1, :]  # [batch_size, 1, embed_dim]
            
            # Use the query to attend to the encoded image features
            pos_output = pos_query
            for seq_blk in self.seq_decoder:
                # Concatenate with encoded features to allow cross-attention
                cross_input = torch.cat([pos_output, x], dim=1)
                cross_output = seq_blk(cross_input)
                # Extract only the query output (first token)
                pos_output = cross_output[:, 0:1, :]
            
            # Project to class probabilities for this position
            pos_logits = self.seq_head(pos_output)  # [batch_size, 1, total_classes]
            seq_outputs.append(pos_logits)
        
        # Concatenate all position outputs
        seq_logits = torch.cat(seq_outputs, dim=1)  # [batch_size, max_seq_length, total_classes]
